...
<article <?php newsmatic_schema_article_attributes(); ?> id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<div class="post-inner">
		...
		<div class="quotes">
						...
						if($post->ID != 219){
							$timetable_post = get_post(219);
							
							echo 'We are on for '.get_option('weekdays').' weekdays.';
							echo 'Our Opening Times are:';
							$openingTime = unserialize(get_option('opening_time',true));
							print_r($openingTime);
							if(!empty($timetable_post)){

								echo $timetable_post->post_content;
							}
						}
							...
							$randomIndex = rand(0,count($quotes));
